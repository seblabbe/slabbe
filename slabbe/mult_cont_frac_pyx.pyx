...
cdef class MCFAlgorithm(object):
    ...
    def coding_iterator(self, start):
        ...
        """
        cdef PairPoint P = PairPoint(d, start)
        cdef int branch
    ...
    def simplex_orbit_iterator(self, start=None, int norm_xyz=0, int norm_uvw=1):
        ...
        cdef int branch
        if start is None:
            P = PairPoint(d, [random() for i in range(d)])
        else:
            P = PairPoint(d, start)
        P.a[0] = 1./3
        P.a[1] = 1./3
    ...
    def _invariant_measure_dict(self, int n_iterations, int ndivs, v=None,
        ...
        cdef PairPoint P = PairPoint(d, [random() for i in range(d)], [0.3]*d)
        P.sort()
    ...
    def _natural_extention_dict(self, int n_iterations, int norm_xyz=0,
        ...
        cdef PairPoint P = PairPoint(d, [random() for i in range(d)], [1/3.]*d)
        P.sort()
        P.normalize_x(1)
        cdef PairPoint R = P.copy()
